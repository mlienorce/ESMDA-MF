...
dims = (23,3,20) # number of cells in each direction (x,y,z)
...
def resample_prior_multiple_1D_to_2D_including_prev_2D(ne, param_vec_1d_list, param_1d_columns, post_param= None, include_1d_estimate = True):
    """Resample a 1D ensemble into 2D fields conditioned on the central column."""
    ...
    twoD_std = [normal_rh_std_2d, ratio_latent_std]

    for param_idx in range(2):
        mean_vec = np.zeros(nz*nx)
        memberwise_trend = np.zeros([nz*nx,Ne_opt])
        all_conditioning_indices = []
        all_conditioning_columns = []
        for column_idx_no, column_idx in enumerate(param_1d_columns):
            param_vec_1d = param_vec_1d_list[column_idx_no]
            conditioning_indices = column_idx_no * nz + np.arange(nz)
            if param_vec_1d.shape != (2 * nz, Ne_opt):
                raise ValueError(
                    f"Expected param_vec_1D with shape ({2 * nz}, {Ne_opt}), got {param_vec_1d.shape}"
                )
            conditioning_column = param_vec_1d[param_idx * nz:(param_idx + 1) * nz, :]
            mean_profile = np.mean(conditioning_column, axis=1)
            if column_idx_no == 0:
                if post_param is None or include_1d_estimate:
                    mean_vec_1d = np.tile(mean_profile, nx)
                    memberwise_trend_1d = np.tile(conditioning_column, (nx, 1))
                else:
                    mean_vec_1d = 0
                    memberwise_trend_1d = 0
                if post_param:
                    reshaped_idx =np.array([])
                    for post_param_one_ens_member in post_param: # initialize with previous 2D results
                        ...
                        reshaped_org = post_param_one_ens_member.reshape(2, dims[0] * dims[2])
                        if not keep_inversion_lim_x:
                            # Alt 1. dublicate last column
                            reshaped = np.concatenate((reshaped_org[:, nz:], reshaped_org[:, -nz:]), axis=1)
                            # Alt 2. make a new guess
                            if apply_bounds:
                                prior_0d, _, _ = sample_prior_0D(100,normal_rh_bounded_mean, normal_rh_bounded_std)
                            else:
                                prior_0d, _, _ = sample_prior_0D(100, normal_rh_mean, normal_rh_std)

                            _, _, mean_prior_1d = resample_prior_0D_to_1D(100, prior_0d)
                            reshaped[:, -nz:] = mean_prior_1d.reshape(2,nz)
                    ...
                    mean_vec_2d = np.mean(reshaped_idx, axis=1)
                    memberwise_trend_2d = reshaped_idx
                else:
                    mean_vec_2d = 0
                    memberwise_trend_2d = 0
                # Can choose to combine 1D and 2d information or apply them as individual ensemble members
                mean_vec = np.mean(np.stack((mean_vec_1d, mean_vec_2d), axis=0), axis=0)
                memberwise_trend = np.mean(np.stack((memberwise_trend_1d, memberwise_trend_2d), axis=0), axis=0)
            mean_vec[conditioning_indices] = mean_profile # averaged over ensemble but only 1D information
            memberwise_trend[conditioning_indices, :] = conditioning_column[:,:]
        ...
        if stationary_std <= np.finfo(float).eps:
            cov = np.eye(n_params_per_type, dtype=float) * np.finfo(float).eps
            conditioned_residuals = np.zeros((n_params_per_type, Ne_opt), dtype=float)
        else:
            cov = geostat.gen_cov2d(
                nx, nz, stationary_std**2, var_range_cells, vertical_aspect, 0, 'exp'
            )
            unconditional_residuals = geostat.gen_real(
                np.zeros(n_params_per_type, dtype=float), cov, Ne_opt
            ).reshape(
                n_params_per_type, Ne_opt, order='C'
            )
...
def disallow_updates_prior_to_bit(search_dir_orig):
...
for el, assim_index in enumerate(tot_assim_index[138:200:assim_step_integer]):#enumerate([tot_assim_index[0]]):
    ...
    if curr_loss > 0:
        ...
        if el == 0:
            if restart_simulations:
                ...
                param_vec = copy.deepcopy(pr[:, order_1d_ensemble_members_after_loss(Ne_opt)])  # Take the Ne samples with the lowes loss in case resampling returns more
        ...
        for ne_count in range(Ne):
            ...
            for iter in range(max_iterations):
                ...
                for _ in range(max_lm_trials):
                    ...
                    search_dir = disallow_updates_prior_to_bit(search_dir)
        ...
        c_theta_prior_rml = copy.deepcopy(c_theta)
        ...
        results_dict = {
            ...
            'prior_covariance_rml': c_theta_prior_rml
        }
        # save to pickle
        ...
        # free memory:
        for name in ("prior_rml", "post_mda", "post_rml", "post_pred",
                         "post_jac", "post_jac_phys",
                         "mean_theta_prior_rml", "c_theta",
                         "pr", "mean_theta", "results_restart"):
            if name in locals():
                try:
                    del locals()[name]
                except Exception:
